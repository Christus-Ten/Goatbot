...
module.exports = {
  ...
  onReply: async function ({ sock, chatId, event, message, senderId, usersData }) {
    ...
    if (senderId !== data.author) {
      return sock.sendMessage(chatId, { text: "❌ Seul le joueur qui a lancé le quiz peut répondre." }, { quoted: event });
    }

    const rawAnswer = (event.message?.conversation || event.message?.extendedTextMessage?.text || "").trim().toUpperCase();

    if (!["A", "B", "C", "D"].includes(rawAnswer) && !data.isFlag && !data.isAnime) {
    ...
    if (timeSpent > 30) {
      ...
      try { await sock.sendMessage(chatId, { delete: { remoteJid: chatId, fromMe: true, id: data.messageID } }); } catch (e) {}
      return sock.sendMessage(chatId, { text: "⏰ Temps écoulé !" }, { quoted: event });
    }

    let isCorrect = false;
    let userAnswer = rawAnswer;
    let correctAnswerText = data.correctAnswer || data.answer;

    if (data.isFlag || data.isAnime) {
      const optionIndex = rawAnswer.charCodeAt(0) - 65;
      if (optionIndex >= 0 && optionIndex < data.options.length) {
        userAnswer = data.options[optionIndex];
        isCorrect = (userAnswer === data.answer);
      } else {
        return sock.sendMessage(chatId, { text: "❌ Option invalide." }, { quoted: event });
      }
    } else {
      if (rawAnswer === data.correctLetter?.toUpperCase()) {
        isCorrect = true;
      } else {
        isCorrect = (rawAnswer.toLowerCase() === data.correctAnswer?.toLowerCase());
      }
    }
    ...
    try {
      ...
      if (result === "correct" || isCorrect) {
        ...
        const dailyBonus = data.isDailyChallenge ? ' 🌟' : '';

        const responseMsg = `🎉 Bonne réponse !\n` +
        ...
        const responseMsg = `❌ Mauvaise réponse ! Bonne réponse : ${correctAnswerText}\n` +
          `📊 Score : ${user.correct || 0}/${user.total || 0} (${user.accuracy || 0}%)\n` +
          `💔 Série réinitialisée\n` +
      ...
      sock.sendMessage(chatId, { text: `⚠️ Erreur lors du traitement.` }, { quoted: event });
    }
  },
  ...
  onReaction: async function ({ sock, event, usersData }) {
    ...
    if (event.senderId !== author || reacted) return;
  ...
  async handleDailyChallenge(sock, chatId, event, senderId, commandName, usersData) {
    try {
      ...
      let correctLetter = "";
      if (options[0] === answer) correctLetter = "a";
      else if (options[1] === answer) correctLetter = "b";
      else if (options[2] === answer) correctLetter = "c";
      else if (options[3] === answer) correctLetter = "d";
